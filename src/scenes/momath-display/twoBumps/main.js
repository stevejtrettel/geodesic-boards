...
const canvas = world.renderer.domElement;
canvas.style.touchAction = 'none'; // Prevent default touch behavior (like pinch-to-zoom)

// --- Manage OrbitControls switching ---
let orbitControlsEnabled = false;

canvas.addEventListener('touchstart', (event) => {
    if (event.touches.length === 1) {
        orbitControlsEnabled = false;
        world.controls.enabled = false; // disable OrbitControls
    } else if (event.touches.length === 2) {
        orbitControlsEnabled = true;
        world.controls.enabled = true; // enable OrbitControls
    }
});
...
const pointerUvListener = makePointerUvListener(
    ...
    board.block,
    (uv /* THREE.Vector2 */) => {
        // Skip if OrbitControls is active
...
canvas.addEventListener('pointerdown', pointerUvListener);
canvas.addEventListener('pointermove', pointerUvListener);
canvas.addEventListener('pointerup',   () => { /* cleanup if needed */ });
canvas.addEventListener('pointercancel', () => { /* cleanup if needed */ });
