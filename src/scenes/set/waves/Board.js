...
export default class Board extends Vignette {
    ...
    constructor() {
        ...
        this.stripes = new GeodesicStripes(surf,10,this.stripeParams,geodesicMaterial);
        ...
        this.download = ()=>{
            ...
            downloadTextFile('Waves.txt',string);
        }
    }
    ...
    addToUI(ui){
        ...
        surface.add(this.params,'a',0,1.5,0.01).name('Height').onChange(value=> {
            this.needsUpdate=true;
        });
        surface.add(this.params,'b',0.,2,0.01).name('Frequency').onChange(value=> {
            this.needsUpdate=true;
        });
