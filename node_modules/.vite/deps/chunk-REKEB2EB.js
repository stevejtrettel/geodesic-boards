...
var require_tiny_emitter = __commonJS({
  "node_modules/tiny-emitter/index.js"(exports, module) {
    ...
    E.prototype = {
      on: function(name119, callback, ctx) {
        var e2 = this.e || (this.e = {});
        (e2[name119] || (e2[name119] = [])).push({
          fn: callback,
          ctx
        });
        return this;
      },
      once: function(name119, callback, ctx) {
        var self2 = this;
        function listener() {
          self2.off(name119, listener);
          callback.apply(ctx, arguments);
        }
        ;
        listener._ = callback;
        return this.on(name119, listener, ctx);
      },
      emit: function(name119) {
        var data = [].slice.call(arguments, 1);
        var evtArr = ((this.e || (this.e = {}))[name119] || []).slice();
        var i = 0;
      ...
      off: function(name119, callback) {
        ...
        liveEvents.length ? e2[name119] = liveEvents : delete e2[name119];
        return this;
      }
    };
    module.exports = E;
...
function validateOption(options, name119, values) {
  if (options[name119] !== void 0 && !values.includes(options[name119])) {
    console.warn('Warning: Unknown value "' + options[name119] + '" for configuration option "' + name119 + '". Available options: ' + values.map((value) => JSON.stringify(value)).join(", ") + ".");
  }
}
...
function factory(name119, dependencies120, create3, meta) {
  function assertAndCreate(scope) {
    var deps = pickShallow(scope, dependencies120.map(stripOptionalNotation));
    assertDependencies(name119, dependencies120, scope);
    return create3(deps);
  }
  assertAndCreate.isFactory = true;
  assertAndCreate.fn = name119;
  assertAndCreate.dependencies = dependencies120.slice().sort();
  if (meta) {
...
function assertDependencies(name119, dependencies120, scope) {
  var allDefined = dependencies120.filter((dependency) => !isOptionalDependency(dependency)).every((dependency) => scope[dependency] !== void 0);
  if (!allDefined) {
    var missingDependencies = dependencies120.filter((dependency) => scope[dependency] === void 0);
    throw new Error('Cannot create function "'.concat(name119, '", ') + "some dependencies are missing: ".concat(missingDependencies.map((d) => '"'.concat(d, '"')).join(", "), "."));
  }
}
...
function create() {
  ...
  function findType(typeName) {
    ...
    const name119 = typeName.toLowerCase();
    let otherName;
    for (otherName of typeList) {
      if (otherName.toLowerCase() === name119) {
  ...
  function findTypeNames(value) {
    const matches = typeList.filter((name119) => {
      const type = typeMap.get(name119);
  ...
  function findSignature(fn, signature, options) {
    ...
    if (exact) {
      remainingSignatures = [];
      let name119;
      for (name119 in fn.signatures) {
        remainingSignatures.push(fn._typedFunctionData.signatureMap.get(name119));
      }
    } else {
  ...
  function mergeExpectedParams(signatures, index) {
    ...
    signatures.forEach((signature) => {
      ...
      let name119;
      for (name119 of paramSet) {
        typeSet.add(name119);
      }
    });
    return typeSet.has("any") ? ["any"] : Array.from(typeSet);
  }
  function createError(name119, args, signatures) {
    let err, expected;
    const _name = name119 || "unnamed";
  ...
  function conflicting(params1, params2) {
    ...
    for (let i = 0; i < ii; i++) {
      ...
      let overlap = false;
      let name119;
      for (name119 of typeSet2) {
        if (typeSet1.has(name119)) {
          overlap = true;
          break;
        }
      }
  ...
  function createTypedFunction(name119, rawSignaturesMap) {
    typed.createCount++;
    ...
    const generic = function generic2() {
      ...
      return typed.onMismatch(name119, arguments, signatures);
    };
  ...
  function _onMismatch(name119, args, signatures) {
    throw createError(name119, args, signatures);
  }
  ...
  function getObjectName(obj) {
    ...
    for (const key in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, key) && (isTypedFunction(obj[key]) || typeof obj[key].signature === "string")) {
        name119 = checkName(name119, obj[key].name);
      }
    }
    return name119;
  }
  ...
  typed = function(maybeName) {
    ...
    for (let i = start; i < arguments.length; ++i) {
      ...
      if (!named) {
        name119 = checkName(name119, thisName);
      }
      mergeSignatures(allSignatures, theseSignatures);
    }
    return createTypedFunction(name119 || "", allSignatures);
  };
  typed.create = create;
...
var createTyped = factory("typed", dependencies, function createTyped2(_ref) {
  ...
  typed.onMismatch = (name119, args, signatures) => {
    var usualError = typed.createError(name119, args, signatures);
    ...
    signatures.some((sig) => !sig.params.includes(","))) {
      var err = new TypeError("Function '".concat(name119, "' doesn't apply to matrices. To call it ") + "elementwise on a matrix 'M', try 'map(M, ".concat(name119, ")'."));
...
var createChainClass = factory(name4, dependencies5, (_ref) => {
  ...
  function createProxy(name119, fn) {
    if (typeof fn === "function") {
      Chain.prototype[name119] = chainify(fn);
    }
  }
  function createLazyProxy(name119, resolver) {
    lazy(Chain.prototype, name119, function outerResolver() {
      var fn = resolver();
      if (typeof fn === "function") {
        return chainify(fn);
      }
  ...
  if (on) {
    on("import", function(name119, resolver, path) {
      if (!path) {
        createLazyProxy(name119, resolver);
      }
    });
  }
  return Chain;
...
var createUtil = factory(name8, dependencies9, (_ref) => {
  ...
  function hasProperty(nodeOrName, property) {
    ...
    var name119 = defaultName;
    if (typeof nodeOrName === "string") {
      name119 = nodeOrName;
    } else if (isOperatorNode(nodeOrName)) {
      name119 = nodeOrName.fn.toString();
    } else if (isFunctionNode(nodeOrName)) {
      name119 = nodeOrName.name;
    } else if (isParenthesisNode(nodeOrName)) {
      name119 = "paren";
    }
    if (hasOwnProperty(context, name119)) {
      var properties2 = context[name119];
      if (hasOwnProperty(properties2, property)) {
        return properties2[property];
      }
      if (hasOwnProperty(defaultContext, name119)) {
        return defaultContext[name119][property];
      }
    }
    ...
    if (hasOwnProperty(defaultContext, name119)) {
      var _properties2 = defaultContext[name119];
...
var name9 = "simplify";
var dependencies10 = ["typed", "parse", "equal", "resolve", "simplifyConstant", "simplifyCore", "AccessorNode", "ArrayNode", "ConstantNode", "FunctionNode", "IndexNode", "ObjectNode", "OperatorNode", "ParenthesisNode", "SymbolNode", "replacer"];
...
var createCeil = factory(name17, dependencies18, (_ref2) => {
  ...
  return typed("ceil", {
    ...
    "Array | Matrix, number | BigNumber, Unit": typed.referToSelf((self2) => (x, n, unit) => {
      return deepMap2(x, (value) => self2(value, n, unit), true);
    }),
    "Array | Matrix | Unit, Unit": typed.referToSelf((self2) => (x, unit) => self2(x, 0, unit)),
...
function recreateFactory(name119, dependencies120, create3) {
  return factory(name119, dependencies120, create3, {
...
var createNumber = factory(name25, dependencies26, (_ref) => {
  ...
  var number = typed("number", {
    ...
    "Array | Matrix": typed.referToSelf((self2) => (x) => deepMap2(x, self2))
  });
    ...
    _compile(math, argNames) {
      ...
      if (!this.index) {
        ...
        return function evalAssignmentNode(scope, args, context) {
          ...
          scope.set(name119, assign(childObject, index, value));
          return value;
        };
      } else {
...
function toSymbol(name119, isUnit2) {
  isUnit2 = typeof isUnit2 === "undefined" ? false : isUnit2;
  if (isUnit2) {
    if (hasOwnProperty(latexUnits, name119)) {
      return latexUnits[name119];
    }
    return "\\mathrm{" + escapeLatex(name119) + "}";
  }
  if (hasOwnProperty(latexSymbols, name119)) {
    return latexSymbols[name119];
  }
  return escapeLatex(name119);
}
...
var createFunctionAssignmentNode = factory(name37, dependencies38, (_ref) => {
  ...
  class FunctionAssignmentNode extends Node {
     ...
     */
    constructor(name119, params, expr) {
      super();
      if (typeof name119 !== "string") {
      ...
      if (keywords.has(name119)) {
        throw new Error('Illegal function name, "' + name119 + '" is a reserved keyword');
      }
    ...
    _compile(math, argNames) {
      ...
      var name119 = this.name;
      var params = this.params;
      var signature = join(this.types, ",");
      var syntax = name119 + "(" + join(this.params, ", ") + ")";
      return function evalFunctionAssignmentNode(scope, args, context) {
        ...
        var fn = typed(name119, signatures);
        fn.syntax = syntax;
        scope.set(name119, fn);
        return fn;
      };
    }
    /**
...
var createSymbolNode = factory(name44, dependencies45, (_ref) => {
  ...
  function isValuelessUnit(name119) {
    return Unit ? Unit.isValuelessUnit(name119) : false;
  }
  class SymbolNode extends Node {
    ...
    _compile(math2, argNames) {
      var name119 = this.name;
      if (argNames[name119] === true) {
        return function(scope, args, context) {
          return getSafeProperty(args, name119);
        };
      } else if (name119 in math2) {
        return function(scope, args, context) {
          return scope.has(name119) ? scope.get(name119) : getSafeProperty(math2, name119);
        };
      } else {
        var isUnit2 = isValuelessUnit(name119);
        return function(scope, args, context) {
          return scope.has(name119) ? scope.get(name119) : isUnit2 ? new Unit(null, name119) : SymbolNode.onUndefinedSymbol(name119);
        };
      }
    }
    /**
     ...
     * @param {string} name
     */
    static onUndefinedSymbol(name119) {
      throw new Error("Undefined symbol " + name119);
    }
    /**
     * Create a clone of this node, a shallow copy
     * @return {SymbolNode}
     ...
     */
    _toHTML(options) {
      var name119 = escape(this.name);
      if (name119 === "true" || name119 === "false") {
        return '<span class="math-symbol math-boolean">' + name119 + "</span>";
      } else if (name119 === "i") {
        return '<span class="math-symbol math-imaginary-symbol">' + name119 + "</span>";
      } else if (name119 === "Infinity") {
        return '<span class="math-symbol math-infinity-symbol">' + name119 + "</span>";
      } else if (name119 === "NaN") {
        return '<span class="math-symbol math-nan-symbol">' + name119 + "</span>";
      } else if (name119 === "null") {
        return '<span class="math-symbol math-null-symbol">' + name119 + "</span>";
      } else if (name119 === "undefined") {
        return '<span class="math-symbol math-undefined-symbol">' + name119 + "</span>";
      }
      return '<span class="math-symbol">' + name119 + "</span>";
    }
    /**
     * Get a JSON representation of the node
     * @returns {Object}
...
var createFunctionNode = factory(name45, dependencies46, (_ref) => {
  ...
  class FunctionNode extends Node {
    ...
    toString(options) {
      var customString;
      var name119 = this.fn.toString(options);
      if (options && typeof options.handler === "object" && hasOwnProperty(options.handler, name119)) {
        customString = options.handler[name119](this, options);
      }
      if (typeof customString !== "undefined") {
        return customString;
      }
  ...
  _defineProperty(FunctionNode, "onUndefinedFunction", function(name119) {
    throw new Error("Undefined function " + name119);
  });
...
var createParse = factory(name46, dependencies47, (_ref) => {
  ...
  function parseAssignment(state) {
    ...
    if (state.token === "=") {
      if (isSymbolNode(node)) {
        ...
        return new AssignmentNode(new SymbolNode(name119), value);
        ...
        if (valid) {
          getTokenSkipNewline(state);
          value = parseAssignment(state);
          return new FunctionAssignmentNode(name119, args, value);
        }
      }
  ...
  function parseShift(state) {
    ...
    while (hasOwnProperty(operators, state.token)) {
      name119 = state.token;
      fn = operators[name119];
      getTokenSkipNewline(state);
      params = [node, parseConversion(state)];
      node = new OperatorNode(name119, fn, params);
    }
    return node;
  }
  ...
  function parseMultiplyDivideModulusPercentage(state) {
    ...
    while (true) {
      if (hasOwnProperty(operators, state.token)) {
        ...
        fn = operators[name119];
        getTokenSkipNewline(state);
        if (name119 === "%" && state.tokenType === TOKENTYPE.DELIMITER && state.token !== "(") {
          if (state.token !== "" && operators[state.token]) {
            ...
            node = new OperatorNode(name119, fn, [left, last]);
          } else {
            node = new OperatorNode("/", "divide", [node, new ConstantNode(100)], false, true);
          }
        } else {
          last = parseImplicitMultiplication(state);
          node = new OperatorNode(name119, fn, [node, last]);
        }
      } else {
        break;
      }
  ...
  function parseUnary(state) {
    ...
    if (hasOwnProperty(operators, state.token)) {
      ...
      return new OperatorNode(name119, fn, params);
    }
  ...
  function parseSymbol(state) {
    ...
    if (state.tokenType === TOKENTYPE.SYMBOL || state.tokenType === TOKENTYPE.DELIMITER && state.token in NAMED_DELIMITERS) {
      name119 = state.token;
      getToken(state);
      if (hasOwnProperty(CONSTANTS, name119)) {
        node = new ConstantNode(CONSTANTS[name119]);
      } else if (NUMERIC_CONSTANTS.includes(name119)) {
        node = new ConstantNode(numeric2(name119, "number"));
      } else {
        node = new SymbolNode(name119);
      }
      node = parseAccessors(state, node);
      return node;
    }
...
var name49 = "Parser";
var dependencies50 = ["evaluate", "parse"];
var createParserClass = factory(name49, dependencies50, (_ref) => {
  ...
  Parser.prototype.get = function(name119) {
    if (this.scope.has(name119)) {
      return this.scope.get(name119);
    }
  };
  ...
  function isValidVariableName(name119) {
    if (name119.length === 0) {
      return false;
    }
    for (var i = 0; i < name119.length; i++) {
      var cPrev = name119.charAt(i - 1);
      var c = name119.charAt(i);
      var cNext = name119.charAt(i + 1);
  ...
  Parser.prototype.set = function(name119, value) {
    if (!isValidVariableName(name119)) {
      throw new Error("Invalid variable name: '".concat(name119, "'. Variable names must follow the specified rules."));
    }
    this.scope.set(name119, value);
    return value;
  };
  Parser.prototype.remove = function(name119) {
    this.scope.delete(name119);
  };
...
var name50 = "mapSlices";
var dependencies51 = ["typed", "isInteger"];
var createMapSlices = factory(name50, dependencies51, (_ref) => {
  var {
    typed,
    isInteger: isInteger2
  } = _ref;
...
function optimizeCallback(callback, array, name119) {
    ...
    if (numberOfArguments >= 1 && numberOfArguments <= 3) {
      return {
        ...
        fn: function fn() {
          ...
          return _tryFunctionWithArgs(fastCallback, args.slice(0, numberOfArguments), name119, callback.name);
        }
      };
    }
    return {
      ...
      fn: function fn() {
        ...
        return _tryFunctionWithArgs(fastCallback, args, name119, callback.name);
      }
    };
  }
  if (isUnary === void 0) {
...
function _forEach(array, callback) {
  var fastCallback = optimizeCallback(callback, array, name52);
...
var createMap2 = factory(name53, dependencies54, (_ref) => {
  ...
  function _mapArray(array, callback) {
    var fastCallback = optimizeCallback(callback, array, name53);
...
var createFactorial = factory(name58, dependencies59, (_ref) => {
  ...
  return typed(name58, {
    ...
    "Array | Matrix": typed.referToSelf((self2) => (n) => deepMap2(n, self2))
  });
});
...
var dependencies64 = ["typed", "config", "log2", "?on"];
...
var dependencies73 = ["typed", "SparseMatrix"];
...
var name73 = "equal";
var dependencies74 = ["typed", "matrix", "equalScalar", "DenseMatrix", "concat", "SparseMatrix"];
...
var name75 = "smaller";
var dependencies76 = ["typed", "config", "bignumber", "matrix", "DenseMatrix", "concat", "SparseMatrix"];
...
var dependencies77 = ["typed", "config", "matrix", "DenseMatrix", "concat", "SparseMatrix"];
...
var name77 = "larger";
var dependencies78 = ["typed", "config", "bignumber", "matrix", "DenseMatrix", "concat", "SparseMatrix"];
...
var name78 = "largerEq";
var dependencies79 = ["typed", "config", "matrix", "DenseMatrix", "concat", "SparseMatrix"];
...
var name80 = "unequal";
var dependencies81 = ["typed", "config", "equalScalar", "matrix", "DenseMatrix", "concat", "SparseMatrix"];
...
var name85 = "max";
var dependencies86 = ["typed", "config", "numeric", "larger", "isNaN"];
...
var name86 = "min";
var dependencies87 = ["typed", "config", "numeric", "smaller", "isNaN"];
...
var name92 = "variance";
var dependencies93 = ["typed", "add", "subtract", "multiply", "divide", "mapSlices", "isNaN"];
...
var dependencies94 = ["typed", "?bignumber", "add", "subtract", "divide", "multiply", "partitionSelect", "compare", "isInteger", "smaller", "smallerEq", "larger", "mapSlices"];
...
var name98 = "mapSlices";
var dependencies99 = ["typed", "isInteger"];
var createMapSlicesTransform = factory(name98, dependencies99, (_ref) => {
  var {
    typed,
    isInteger: isInteger2
  } = _ref;
...
function compileInlineExpression(expression, math, scope) {
  ...
  var name119 = symbol.name;
  var argsScope = /* @__PURE__ */ new Map();
  var subScope = new PartitionedMap(scope, argsScope, /* @__PURE__ */ new Set([name119]));
  var eq = expression.compile();
  return function inlineExpression(x) {
    argsScope.set(name119, x);
    return eq.evaluate(subScope);
  };
}

// node_modules/mathjs/lib/esm/expression/transform/utils/transformCallback.js
var name99 = "transformCallback";
var dependencies100 = ["typed"];
var createTransformCallback = factory(name99, dependencies100, (_ref) => {
  var {
    typed
  } = _ref;
...
var createFilterTransform = factory(name100, dependencies101, (_ref) => {
  var {
    typed
  } = _ref;
...
var createForEachTransform = factory(name101, dependencies102, (_ref) => {
  var {
    typed
  } = _ref;
...
var name102 = "map";
var dependencies103 = ["typed"];
var createMapTransform = factory(name102, dependencies103, (_ref) => {
  var {
    typed
  } = _ref;
  var map2 = createMap2({
...
var name103 = "max";
var dependencies104 = ["typed", "config", "numeric", "larger", "isNaN"];
var createMaxTransform = factory(name103, dependencies104, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
...
var name104 = "mean";
var dependencies105 = ["typed", "add", "divide"];
var createMeanTransform = factory(name104, dependencies105, (_ref) => {
  var {
    typed,
    add: add2,
    divide: divide2
...
var name105 = "min";
var dependencies106 = ["typed", "config", "numeric", "smaller", "isNaN"];
var createMinTransform = factory(name105, dependencies106, (_ref) => {
  var {
    typed,
    config: config4,
    numeric: numeric2,
...
var name106 = "range";
var dependencies107 = ["typed", "config", "?matrix", "?bignumber", "smaller", "smallerEq", "larger", "largerEq", "add", "isPositive"];
var createRangeTransform = factory(name106, dependencies107, (_ref) => {
  var {
    typed,
    config: config4,
    matrix,
...
var name107 = "std";
var dependencies108 = ["typed", "map", "sqrt", "variance"];
var createStdTransform = factory(name107, dependencies108, (_ref) => {
  var {
    typed,
    map: map2,
    sqrt: sqrt2,
...
var name108 = "sum";
var dependencies109 = ["typed", "config", "add", "numeric"];
var createSumTransform = factory(name108, dependencies109, (_ref) => {
  var {
    typed,
    config: config4,
    add: add2,
...
var name109 = "cumsum";
var dependencies110 = ["typed", "add", "unaryPlus"];
var createCumSumTransform = factory(name109, dependencies110, (_ref) => {
  var {
    typed,
    add: add2,
    unaryPlus
...
var name110 = "variance";
var dependencies111 = ["typed", "add", "subtract", "multiply", "divide", "mapSlices", "isNaN"];
var createVarianceTransform = factory(name110, dependencies111, (_ref) => {
  var {
    typed,
    add: add2,
    subtract,
...
var createClone = factory(name111, dependencies112, (_ref) => {
  var {
    typed
  } = _ref;
...
var name112 = "isNumeric";
var dependencies113 = ["typed"];
var createIsNumeric = factory(name112, dependencies113, (_ref) => {
  var {
    typed
  } = _ref;
...
var name113 = "hasNumericValue";
var dependencies114 = ["typed", "isNumeric"];
var createHasNumericValue = factory(name113, dependencies114, (_ref) => {
  var {
    typed,
    isNumeric
  } = _ref;
...
var createTypeOf = factory(name114, dependencies115, (_ref) => {
  var {
    typed
  } = _ref;
...
var createIsPrime = factory(name115, dependencies116, (_ref) => {
  var {
    typed
  } = _ref;
...
var createNumeric = factory(name116, dependencies117, (_ref) => {
  var {
    number: _number,
    bignumber,
    fraction
...
var name117 = "reviver";
var dependencies118 = ["classes"];
var createReviver = factory(name117, dependencies118, (_ref) => {
  var {
    classes
  } = _ref;
...
var name118 = "replacer";
var dependencies119 = [];
var createReplacer = factory(name118, dependencies119, () => {
...
function importFactory(typed, load, math, importedFactories) {
  function mathImport(functions, options) {
    ...
    function flattenImports(flatValues2, value2, name120) {
      ...
      } else if (isFactory(value2) || name120 !== void 0) {
        var flatName = isFactory(value2) ? isTransformFunctionFactory(value2) ? value2.fn + ".transform" : value2.fn : name120;
    ...
    for (var name119 in flatValues) {
      if (hasOwnProperty(flatValues, name119)) {
        var value = flatValues[name119];
        if (isFactory(value)) {
          _importFactory(value, options);
        } else if (isSupportedType(value)) {
          _import(name119, value, options);
        } else {
          if (!options.silent) {
            throw new TypeError("Factory, Object, or Array expected");
          }
  ...
  function _import(name119, value, options) {
    var _math$Unit;
    ...
    if (typed.isTypedFunction(math[name119]) && typed.isTypedFunction(value)) {
      if (options.override) {
        value = typed(name119, value.signatures);
      } else {
        value = typed(math[name119], value);
      }
      math[name119] = value;
      delete importedFactories[name119];
      _importTransform(name119, value);
      math.emit("import", name119, function resolver() {
        return value;
      });
      return;
    }
    var isDefined = math[name119] !== void 0;
    var isValuelessUnit = (_math$Unit = math.Unit) === null || _math$Unit === void 0 ? void 0 : _math$Unit.isValuelessUnit(name119);
    ...
    if (!options.silent) {
      throw new Error('Cannot import "' + name119 + '": already exists');
    }
  }
  function _importTransform(name119, value) {
    if (value && typeof value.transform === "function") {
      math.expression.transform[name119] = value.transform;
      if (allowedInExpressions(name119)) {
        math.expression.mathWithTransform[name119] = value.transform;
      }
    } else {
      delete math.expression.transform[name119];
      if (allowedInExpressions(name119)) {
        math.expression.mathWithTransform[name119] = value;
      }
    }
  }
  function _deleteTransform(name119) {
    delete math.expression.transform[name119];
    if (allowedInExpressions(name119)) {
      math.expression.mathWithTransform[name119] = math[name119];
    } else {
      delete math.expression.mathWithTransform[name119];
    }
  }
  function _wrap(fn) {
  ...
  function _importFactory(factory2, options) {
    ...
    var name119 = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : factory2.fn;
    if (name119.includes(".")) {
      throw new Error("Factory name should not contain a nested path. Name: " + JSON.stringify(name119));
    }
    var namespace = isTransformFunctionFactory(factory2) ? math.expression.transform : math;
    var existingTransform = name119 in math.expression.transform;
    var existing = hasOwnProperty(namespace, name119) ? namespace[name119] : void 0;
    var resolver = function resolver2() {
      ...
      factory2.dependencies.map(stripOptionalNotation).forEach((dependency) => {
        ...
        if (dependency === "math") {
          dependencies120.math = math;
        } else if (dependency === "mathWithTransform") {
          dependencies120.mathWithTransform = math.expression.mathWithTransform;
        } else if (dependency === "classes") {
          dependencies120.classes = math;
        } else {
          dependencies120[dependency] = math[dependency];
        }
      });
      var instance = factory2(dependencies120);
    ...
    if (!factory2.meta || factory2.meta.lazy !== false) {
      lazy(namespace, name119, resolver);
      if (former) lazy(namespace, former, resolver);
      if (existing && existingTransform) {
        _deleteTransform(name119);
        if (former) _deleteTransform(former);
      } else {
        if (needsTransform) {
          lazy(withTransform, name119, () => namespace[name119]);
          if (former) lazy(withTransform, former, () => namespace[name119]);
        }
      }
    } else {
      namespace[name119] = resolver();
      if (former) namespace[former] = namespace[name119];
      if (existing && existingTransform) {
        _deleteTransform(name119);
        if (former) _deleteTransform(former);
      } else {
    ...
    importedFactories[name119] = factory2;
    math.emit("import", name119, resolver);
  }
  ...
  function allowedInExpressions(name119) {
    return !hasOwnProperty(unsafe, name119);
  }
...
decimal.js/decimal.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-REKEB2EB.js.map
